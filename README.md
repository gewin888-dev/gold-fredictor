...
评分由实际利率、名义利率、联邦基金利率、美元指数、VIX、通胀预期和黄金价格趋势组成。该评分仅用于数据分析和风险提示。

如果数据库中存在可信且未过期的 CFTC 黄金期货持仓，评分会额外加入 `CFTC投机仓位` 因子。中国黄金溢价、央行购金、新闻情绪目前也有因子接口，但 `SAMPLE`、`ESTIMATE`、`MANUAL`、`JSON` 等样本或占位来源默认不纳入评分，只在风险提示中说明。生产模式下页面默认隐藏或弱化这些低可信来源。
